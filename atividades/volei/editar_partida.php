...
if (isset($_POST['atualizar_partida'])) {
    ...
    $genero = $_POST['genero'];
    $fase = $_POST['fase']; // Capturando a nova fase editada
    $data_partida = $_POST['data_partida']; // Capturando a nova data editada
    ...
    if ($id_casa == $id_fora) {
        ...
        try {
            // Atualizando todos os dados antigos + os novos campos (fase e data)
...
// 2. Buscar os dados atuais desta partida específica para preencher os inputs automaticamente
...
// Buscar todos os países para os seletores (<select>) de times
...
<head>
    ...
    <style>
        ...
        button { background: #ffb703; color: #000; border: none; padding: 12px; width: 100%; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 15px; }
...
<body>
    ...
    <script>
    // Mantém a regra de inversão automática ativa também na tela de edição para evitar erros humanos
    ...
    // Executa uma vez no início para travar os times já selecionados originalmente
